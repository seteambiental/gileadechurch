...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.impacto_eventos WHERE id = NEW.evento_id) THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      -- Já existe um evento com o mesmo título no Impacto. Cria o espelho com um
      -- título diferenciado para não colidir com a restrição unique de título.
      INSERT INTO public.impacto_eventos (id, titulo, data_inicio, data_fim, tipo, local, limite_vagas)
      SELECT a.id,
             COALESCE(a.titulo, 'Evento') || ' (' || left(a.id::text, 8) || ')',
             COALESCE(a.data_evento, CURRENT_DATE),
             a.data_fim,
             COALESCE(a.tipo_evento, 'evento'),
             a.local,
...
-- Remove inscrições de teste criadas durante a verificação do problema
DELETE FROM public.impacto_inscricoes
WHERE evento_id = '4ad4ae74-7e9e-4aae-b346-04f5f4d09fac';