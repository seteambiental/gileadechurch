...
serve(async (req) => {
  ...
  try {
    ...
    const { user_id, email } = parsedBody.data;
    ...
    // Atualiza o email de login já confirmado (sem exigir verificação por email)
    ...
    console.log(`Email de login atualizado para user_id=${user_id} por ${user.id}`);

    return new Response(
      JSON.stringify({ success: true, message: "Email de login atualizado" }),
