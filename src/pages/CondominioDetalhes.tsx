...
import { ArrowLeft, Loader2, Building, Users, Package, DollarSign, Home, UserCheck } from "lucide-react";
...
const CondominioDetalhes = () => {
  ...
  const condominioNome = decodeURIComponent(nome || "");
  ...
  const { data: casas = [], isLoading: loadingCasas } = useQuery({
    queryKey: ["casas-condominio", condominioNome],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("casas_refugio")
        .select("*")
        .eq("condominio", condominioNome)
        .order("name");
      if (error) throw error;
      return data;
    },
    enabled: !!condominioNome,
  });

  const casaIds = useMemo(() => casas.map(c => c.id), [casas]);
  const supervisores = useMemo(() => [...new Set(casas.map(c => c.supervisores).filter(Boolean))], [casas]);

  const { data: encontros = [], isLoading: loadingEncontros } = useQuery({
    queryKey: ["encontros-condominio", casaIds],
    queryFn: async () => {
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 bg-card/95 backdrop-blur border-b border-border">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-heading font-bold text-lg text-foreground">{condominioNome}</h1>
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <Card className="bg-card border-border">
            <CardContent className="pt-4 pb-4 text-center">
              <p className="text-2xl font-bold text-foreground">{totalSupervisores}</p>
...
export default CondominioDetalhes;
