...
import { Loader2, FileSpreadsheet, FileText, Download, Calendar } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
export const EncontrosReportDialog = ({
  ...
  const { data: encontros = [], isLoading } = useQuery({
    queryKey: ["encontros-report", casasIds, appliedStartDate, appliedEndDate],
    queryFn: async () => {
      if (casasIds.length === 0) return [];

      let query = supabase
        .from("encontros_casa_refugio")
        .select("*")
        .in("casa_refugio_id", casasIds)
