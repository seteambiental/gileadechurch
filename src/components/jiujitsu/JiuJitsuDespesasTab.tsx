...
const JiuJitsuDespesasTab = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        const { error } = await supabase.from("jiujitsu_despesas").update(payload).eq("id", editingId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("jiujitsu_despesas").insert(payload);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jiujitsu-despesas"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("jiujitsu_despesas").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jiujitsu-despesas"] });
  ...
  const handleExportExcel = async () => {
    ...
    await exportGenericToExcel(despesas, exportColumns, "Despesas_JiuJitsu", "Despesas");
  };
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Despesas — Jiu-Jitsu", 14, 18);
    doc.setFontSize(9);
    ...
    savePDF(doc, "Despesas_JiuJitsu.pdf");
  };
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            Nenhuma despesa registrada para o Jiu-Jitsu.
          </CardContent>
        </Card>
      ) : (
        <Card>
...
export default JiuJitsuDespesasTab;
