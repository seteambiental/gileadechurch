...
const TeologiaDespesasTab = () => {
  ...
  const { data: despesas = [], isLoading } = useQuery({
    queryKey: ["teologia-despesas"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("teologia_despesas")
        .select("*")
        .order("categoria");
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) {
        const { error } = await supabase.from("teologia_despesas").update(payload).eq("id", editingId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("teologia_despesas").insert(payload);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teologia-despesas"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("teologia_despesas").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teologia-despesas"] });
  ...
  const handleExportExcel = async () => {
    ...
    await exportGenericToExcel(despesas, exportColumns, "Despesas_Teologia", "Despesas");
  };
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Despesas — Curso de Teologia", 14, 18);
    doc.setFontSize(9);
    ...
    savePDF(doc, "Despesas_Teologia.pdf");
  };
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            Nenhuma despesa registrada para o Curso de Teologia.
          </CardContent>
        </Card>
      ) : (
        <Card>
...
export default TeologiaDespesasTab;
